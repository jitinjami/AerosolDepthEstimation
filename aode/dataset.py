...
class SolaTrainDataset(Dataset):
    ...
    '''
    def __init__(self, path_csv: str, dir_img: str, cache: bool = False, transform = None) -> None:
        super().__init__()
        ...
        self.image_paths = [os.path.join(dir_img, image_name) for image_name in self.csv_data[:, 0]]

        # Get aod values and convert to tensor
        self.aod_values = torch.from_numpy(self.csv_data[:, -1].astype(np.float32))

        # If Cache is activated, images and associated attributes are read into memory
        if self.cache:
            self.images =  torch.empty((len(self.image_paths), 13, 128, 128), dtype=torch.float32)
    ...
    def __getitem__(self, index: int) -> Any:
        ...
        if self.cache:
            ...
            image = torch.tensor(np.array(rasterio.open(self.image_paths[index]).read()), dtype=torch.float32)
...
class SolaTestDataset(Dataset):
    ...
    transform : callable, optional
        Optional transform to be applied to the images (default is None).
    '''
    def __init__(self, path_csv: str, dir_img: str, cache: bool = False) -> None:
        super().__init__()
